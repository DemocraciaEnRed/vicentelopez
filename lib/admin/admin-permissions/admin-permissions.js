...
export default class AdminPermissions extends view('appendable', 'emptyable', 'loadable', 'withEvents') {
  constructor (options = {}) {
    ...
    userStore.findAll().then(this.downloadUsers.bind(this)).catch(err=>console.log(err))
  }

  downloadUsers(users){
    
    let data = [['Nombre','Apellido', 'Email']]

    users.forEach(user =>data.push([user.firstName, user.lastName,user.email]))
    ...
    const tempLink = document.querySelector('.btn-download');
