...
import DataFile from '../admin-data-file/view'
...
page(
  ...
  findPrivateTopics,
  findUniqTopicAttrs('anio', 'barrio', 'state'),
  (ctx) => {
...
page(
  urlBuilder.for('admin.data-file'),
  (ctx) => {
    const settingsStage = new DataFile(ctx.forum)
    settingsStage.replace('.admin-content')
    ctx.sidebar.set('data-file')
  }
)
