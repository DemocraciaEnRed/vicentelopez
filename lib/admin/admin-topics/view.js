...
export default class TopicsListView extends View {
  ...
  switchOn () {
    ...
    //this.bind('click', 'button.publish-topic', this.bound('onpublishclick'))
    //this.bind('click', 'button.unpublish-topic', this.bound('onunpublishclick'))
  ...
  onunpublishclick(ev){
    ev.preventDefault()
    let el = ev.delegateTarget
    let topicid = el.getAttribute('data-topicid')

    el.addClass('disabled')

    topicStore.publish(topicid)
      .then(() => {
